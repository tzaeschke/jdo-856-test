...
/**
 * <B>Title:</B> apache.jdo.tck.SampleReadQueries <br>
...
public class SampleReadQueries3 extends QueryTest {

  /** */
  private static final String ASSERTION_FAILED = "Assertion (apache.jdo.tck.SampleReadQueries) failed: ";
  ...
  private static final String SINGLE_STRING_QUERY_09 =
      "select firstname, salary, manager as reportsTo into org.apache.jdo.tck.query.api.apache.jdo.tck.SampleReadQueries$Info "
  ...
  private static final String SINGLE_STRING_QUERY_10 =
      "select new org.apache.jdo.tck.query.api.apache.jdo.tck.SampleReadQueries$Info (firstname, salary, manager) "
          + "from org.apache.jdo.tck.pc.company.FullTimeEmployee where department.name == :deptName";
  ...
  private static final String SINGLE_STRING_QUERY_16 =
      "select into org.apache.jdo.tck.query.api.apache.jdo.tck.SampleReadQueries$EmpWrapper "
          + "from org.apache.jdo.tck.pc.company.FullTimeEmployee where salary > :sal";

  private static final String SINGLE_STRING_QUERY_17 =
      "select into org.apache.jdo.tck.query.api.apache.jdo.tck.SampleReadQueries$EmpInfo "
          + "from org.apache.jdo.tck.pc.company.FullTimeEmployee where salary > :sal";
  ...
  public void testQuery10a() {
    ...
    try {
      ...
      try (Query<FullTimeEmployee> q =
          pm.newQuery(FullTimeEmployee.class, "department.name == deptName")) {
        q.setResult(
            "new org.apache.jdo.tck.query.api.apache.jdo.tck.SampleReadQueries$Info(firstname, salary, manager)");
        q.declareParameters("String deptName");
        List<Info> infos = (List<Info>) q.execute("R&D");
...
//        checkQueryResultWithoutOrder(ASSERTION_FAILED, SINGLE_STRING_QUERY_13, row, expectedRow);
  ...
  private final HashMap<Thread, MyTimerTask> timers = new HashMap<>();
  ...
  private class MyTimerTask extends TimerTask{
    ...
    public void run() {
      ...
      logger.debug("------------- interrupting: " + id + "   " + thread);
  ...
  @BeforeEach
  public void localBeforeEach() {

    synchronized (timers) {
      Thread t = Thread.currentThread();
      logger.debug("------------- local set-up --------------------------- " + t);
      MyTimerTask tt = new MyTimerTask(t);
      timers.put(t, tt);
      timer.schedule(tt, 30_000);
    }
  }

  @AfterEach
  public void localAfterEach() {
    synchronized (timers) {
      Thread t = Thread.currentThread();
      logger.debug("------------- local tear-down --------------------------- " + t);
      MyTimerTask tt = timers.get(t);
      if (tt != null) {
        tt.thread = null;
        tt.cancel();
      }
...
// TODO?    addTearDownClass(CompanyModelReader.getTearDownClasses());
...
//  }
  ...
  private <T> List<T> getTransientCompanyModelInstancesAsList(Class<T> fullTimeEmployeeClass, String emp1) {
    // TODO dummy
    return new ArrayList<>();
  }


  private <T> T getTransientCompanyModelInstance(Class<T> employeeClass, String emp2) {
    return null;
  }

  private <T> List<T> getTransientCompanyModelInstancesAsList(Class<T> fullTimeEmployeeClass, String emp1, String emp5) {
    // TODO dummy
    return new ArrayList<>();
  }


  private <T> List<T> getTransientCompanyModelInstancesAsList(Class<T> fullTimeEmployeeClass, String emp1, String emp2, String emp5) {
    // TODO dummy
    return new ArrayList<>();
  }
